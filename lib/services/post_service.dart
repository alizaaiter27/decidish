...
import 'package:decidish/services/api_locale_params.dart';
import 'package:decidish/services/api_service.dart';

class PostService {
  static Future<List<dynamic>> getPosts() async {
    final res = await ApiService.get(
      ApiLocaleParams.withMealContentLang(ApiConfig.posts),
      requireAuth: true,
    );
    final raw = res['posts'];
  ...
  static Future<List<dynamic>> getPostsForFriend(String userId) async {
    final res = await ApiService.get(
      ApiLocaleParams.withMealContentLang('${ApiConfig.posts}/user/$userId'),
      requireAuth: true,
    );
    final raw = res['posts'];
    if (raw is! List) return [];
  ...
  static Future<Map<String, dynamic>> likePost(String postId) async {
    final res = await ApiService.post(
      ApiLocaleParams.withMealContentLang('${ApiConfig.posts}/$postId/like'),
      {},
      requireAuth: true,
    );
